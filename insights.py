...
def getLatestPost():
    ...
    print("\n---- LATEST POST -----\n")  # section header
    print("\tLink to post:", response['json_data']['data'][0]['permalink'])
    print("\n\tPost caption:", response['json_data']['data'][0]['caption'])
    print("\n\tMedia Type:", response['json_data']['data'][0]['media_type'])
    print("\n\tPosted at:", response['json_data']['data'][0]['timestamp'])

    return response['json_data']['data']


def getPostInsights(latest_media_id, media_type):
    params = getCreds()  # get creds
    params['latest_media_id'] = latest_media_id  # store latest post id

    if 'VIDEO' == media_type:  # media is a video
    ...
    for insight in response['json_data']['data']:  # loop over post insights
        print(
            "\t" + insight['title'] + " (" + insight['period'] + "): " + str(
                insight['values'][0]['value']))  # display info
...
if __name__ == "__main__":
    ...
    for insight in response['json_data']['data']:  # loop over user account insights
        ...
        for value in insight['values']:  # loop over each value
            print(
                "\t\t" + value['end_time'] + ": " + str(value['value']))  # print out counts for the date
